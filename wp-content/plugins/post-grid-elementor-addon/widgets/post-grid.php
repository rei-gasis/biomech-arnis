...
class Elementor_Post_Grid_Widget extends Widget_Base {
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'6' => esc_html__( 'Landing Page - Side', 'landing-page-side-layout' ),
					'7' => esc_html__( 'Landing Page - Main', 'landing-page-main-layout' ),
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'taxonomy' => __( 'Taxonomy', 'post-grid-elementor-addon' ),
				],
				'separator' => 'before',
			]
		);
			...
			<div class="wpcap-grid-container elementor-grid <?php echo $columns_desktop.$columns_tablet.$columns_mobile.$grid_class; ?>">
				...
				if( 6 == $grid_style) { //side
					$query_args = array(
					        	...
					        	'category_name' 		=> $cats,
					        	'post_type' => array('news', 'updates'), //winson
					        	'offset'   => 1
				        	);
				}elseif( 7 == $grid_style) { //main
					$query_args = array(
					        	'posts_per_page' 		=> absint( $posts_per_page ),
					        	'no_found_rows'  		=> true,
					        	'post__not_in'          => get_option( 'sticky_posts' ),
		        ...
		        if ( $all_posts->have_posts() ) :
					...
					if( 6 == $grid_style ){ //winson

						include( __DIR__ . '/layouts/landing-page-side-layout.php' );						
					}elseif( 7 == $grid_style ){

		        		include( __DIR__ . '/layouts/landing-page-main-layout.php' );
	...
	protected function render_meta() {
		...
		<div class="post-grid-meta">
			...
			if ( in_array( 'taxonomy', $meta_data ) ) { ?>
				
				<span class="post-taxonomy"><?php echo ucwords(get_post_type()); ?></span>
